...
bot.onText(/\Курсы валют/, function (msg, match) {
  ...
  var resp = "Курс валют на "+curs.update+"\n";
    resp += "```"+String("ПОКУПКА      ").slice(0,10)+String("ПРОДАЖА      ").slice(0,10)+String("ЦБ        ").slice(0,10)+"```";
    for (var atr in curs.bank_currency){
    resp += "\n";
    resp += "```" + atr +
       String(curs.bank_currency[atr].buy   + "          ").slice(0,10) +
       String(curs.bank_currency[atr].sell  + "          ").slice(0,10) +
       String(curs.bank_currency[atr].cb    + "          ").slice(0,10) +
       "```";
  }
  bot.sendMessage(fromId,resp,menu.main);
});
